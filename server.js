...
// Lit et nettoie les champs d'un formulaire d'anime (jamais de valeur null)
const readAnimeForm = (body) => {
  const year = parseInt(body.year, 10);
  return {
    title: clean(body.title),
    synopsis: clean(body.synopsis),
    genres: clean(body.genres),
    year: year >= 1900 && year <= 2100 ? year : null,
    status: ['En cours', 'Terminé'].includes(body.status) ? body.status : 'En cours',
    cover: clean(body.cover)
  };
};
...
app.get('/', (req, res) => {
  ...
  if (genre) { sql += " AND (',' || REPLACE(COALESCE(a.genres, ''), ', ', ',') || ',') LIKE ?"; params.push(`%,${genre},%`); }
...
app.get('/admin', requireAdmin, (req, res) => {
  ...
  res.render('admin', { animes, stats, ok: req.query.ok || null });
});
...
app.post('/admin/anime', requireAdmin, (req, res) => {
  const f = readAnimeForm(req.body);
  const fail = (msg) => res.render('admin-anime-form', { anime: { ...f, id: null }, error: msg });
  if (!f.title) return fail('Le titre est obligatoire');
  if (f.cover && !isUrl(f.cover)) return fail('Le lien de la couverture doit commencer par http:// ou https://');
  try {
    const info = db.prepare(`INSERT INTO animes (title, synopsis, genres, year, status, cover)
                             VALUES (?, ?, ?, ?, ?, ?)`)
      .run(f.title, f.synopsis, f.genres, f.year, f.status, f.cover || null);
    backup.scheduleBackup();
    res.redirect('/admin/anime/' + info.lastInsertRowid + '/episodes?ok=' + encodeURIComponent('Anime enregistré. Ajoute maintenant ses épisodes.'));
  } catch (e) {
    console.error('Création anime échouée :', e.message);
    fail('Enregistrement impossible : ' + e.message);
  }
});
...
app.post('/admin/anime/:id', requireAdmin, (req, res) => {
  const f = readAnimeForm(req.body);
  const fail = (msg) => res.render('admin-anime-form', { anime: { ...f, id: req.params.id }, error: msg });
  if (!f.title) return fail('Le titre est obligatoire');
  if (f.cover && !isUrl(f.cover)) return fail('Le lien de la couverture doit commencer par http:// ou https://');
  try {
    db.prepare(`UPDATE animes SET title=?, synopsis=?, genres=?, year=?, status=?, cover=? WHERE id=?`)
      .run(f.title, f.synopsis, f.genres, f.year, f.status, f.cover || null, req.params.id);
    backup.scheduleBackup();
    res.redirect('/admin?ok=' + encodeURIComponent('Anime modifié'));
  } catch (e) {
    console.error('Modification anime échouée :', e.message);
    fail('Enregistrement impossible : ' + e.message);
  }
});
...
app.get('/admin/anime/:id/episodes', requireAdmin, (req, res) => {
  ...
  if (!anime) return res.status(404).send('Introuvable');
  const episodes = db.prepare('SELECT * FROM episodes WHERE anime_id = ? ORDER BY number').all(anime.id);
  res.render('admin-episodes', { anime, episodes, error: req.query.error || null, ok: req.query.ok || null });
});
